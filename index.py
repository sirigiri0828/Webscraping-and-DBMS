...
def update_graph(select_years, radio_items, radio_items1):
    ...
    sales2 = sales1[(sales1['Year'] == select_years) & (sales1['Segment'] == radio_items)].sort_values(by = ['Sales'], ascending = False).nlargest(5, columns = ['Sales'])
...
def update_graph(select_years, radio_items, radio_items2):
    ...
    sales13 = sales12[(sales12['Year'] == select_years) & (sales12['Segment'] == radio_items)].sort_values(by = ['Sales'], ascending = False).nlargest(10, columns = ['Sales'])
    sales14 = sales.groupby(['Year', 'City', 'Segment'])['Sales'].sum().reset_index()
    sales15 = sales14[(sales14['Year'] == select_years) & (sales14['Segment'] == radio_items)].sort_values(by=['Sales'],ascending=False).nlargest(10, columns = ['Sales'])
